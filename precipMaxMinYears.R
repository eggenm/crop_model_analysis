...
#     
#    Assumes a file called millet_factors_FULLRUN.csv exists
#
#
#############################
library(tidyr)
...
summary<-read.csv('millet_factors_FULLRUN.csv')
######################################################################################



summary$precip<-summary$june_precip+summary$july_precip+summary$august_precip+summary$sept_precip
precip_by_year<-summary%>%group_by(zone, year)%>%summarise(average_precip=mean(precip))
...
summary<-summary%>%filter(zone %in% c('Bassin arachidier', 'Casamance', 'Senegal oriental') )
summary_median<-get_median_yearly_observations(summary)
summary_median$yield = 100*summary_median$season_yield

summary_median_old<-summary%>%group_by(soil_id, zone, year)%>%summarise(yield=100*median(season_yield), upper_60_sand_ave = max(upper_60_sand_ave), upper_60_silt_ave = max(upper_60_silt_ave), upper_60_clay_ave = max(upper_60_clay_ave), upper_60_soc_ave=max(upper_60_soc_ave) )
